...
class LBGCN(object):
    def __init__(self, data_config, pretrain_data):
        ...
        self.plain_adj = data_config['plain_adj'] # 没加自环的普通adj
        ...
        *********************************************************
        ...
        self.neg_items = tf.placeholder(tf.int32, shape=(None,))  # 此函数用于定义过程，在执行的时候再赋具体的值
        ...
        *********************************************************
        ...
        Different Convolutional Layers:
            1. ngcf: defined in 'Neural Graph Collaborative Filtering神经图协同过滤', SIGIR2019;
            2. gcn:  defined in 'Semi-Supervised Classification with Graph Convolutional Networks基于图卷积神经网络的半监督节点分类', 
            ICLR2018;
            3. gcmc: defined in 'Graph Convolutional Matrix Completion图卷积矩阵补全', KDD2018;
        """
        ...
        *********************************************************
        ...
        '''tf.nn.embedding_lookup(tenfor,id)就是根据id，寻找embeddings中的第id行。比如input_ids=[1,3,
        5]，则找出embeddings中第1，3，5行，组成一个tensor返回'''
    ...
    def _split_A_hat(self, X):
        ...
        fold_len = (self.n_users + self.n_items) // self.n_fold  # self.n_fold值为100
    ...
    #     '''问题出在这里'''
    #     B_adj = get_N_all(self.plain_adj, self.n_users, self.n_items)
    #
    ...
    #         # out_bi = BILinear_pooling()
    #         out_bi = dot(B_adj, BILinear_pooling(A_fold_hat, side_embeddings), True)
    #         ego_embeddings = (1 - self.alpha) * side_embeddings + self.alpha * out_bi
    #
    #         all_embeddings += [ego_embeddings]
    #
    ...
    # 正在改进的lightgcn
    def _create_lightgcn_embed(self):
        # pre_adj_loop = normalize_adj(self.plain_adj)
        if self.node_dropout_flag:
            A_fold_hat = self._split_A_hat_node_dropout(self.norm_adj)
            # adj_fold_hat = self._split_A_hat_node_dropout(pre_adj_loop)
            adj_fold_hat = self._split_A_hat_node_dropout(self.plain_adj+sp.eye(self.plain_adj.shape[0]))
        else:
            A_fold_hat = self._split_A_hat(self.norm_adj)
            # adj_fold_hat = self._split_A_hat(pre_adj_loop)
            adj_fold_hat = self._split_A_hat(self.plain_adj+sp.eye(self.plain_adj.shape[0]))

        ego_embeddings = tf.concat([self.weights['user_embedding'], self.weights['item_embedding']], axis=0)
        all_embeddings = [ego_embeddings]

        out_bi = BILinear_pooling(adj_fold_hat, ego_embeddings, self.n_fold, self.n_users, self.n_items, self.plain_adj)
        for k in range(0, self.n_layers):
        ...
        all_embeddings = (1 - self.alpha) * all_embeddings + self.alpha * out_bi
        '''tf.reduce_mean 函数用于计算张量tensor沿着指定的数轴（tensor的某一维度）上的的平均值，主要用作降维或者计算tensor（图像）的平均值'''
        u_g_embeddings, i_g_embeddings = tf.split(all_embeddings, [self.n_users, self.n_items], 0)
        return u_g_embeddings, i_g_embeddings
    ...
    #     else:
    #         A_fold_hat = self._split_A_hat(self.norm_adj)
    #     print('A_fold_hat:',A_fold_hat)
    #     ego_embeddings = tf.concat([self.weights['user_embedding'], self.weights['item_embedding']], axis=0)
    #     '''用来将两个张量拼接为一个张量的的函数'''
    #     all_embeddings = [ego_embeddings]
    #
    #     for k in range(0, self.n_layers): # 3 层嵌入
    #
    #         temp_embed = []
    #         for f in range(self.n_fold):
    #             temp_embed.append(tf.sparse_tensor_dense_matmul(A_fold_hat[f], ego_embeddings)) # 邻接矩阵乘上特征矩阵
    #
    #         side_embeddings = tf.concat(temp_embed, 0)
    #         ego_embeddings = side_embeddings
    #         all_embeddings += [ego_embeddings]
    #
    #     all_embeddings = tf.stack(all_embeddings, 1) # 也是张量拼接，只不过两个张量拼接后放在一个[]里用逗号隔开
    #     all_embeddings = tf.reduce_mean(all_embeddings, axis=1) # 在[a,b,c]   (a+b+c)/3 行上求平均
    #     u_g_embeddings, i_g_embeddings = tf.split(all_embeddings, [self.n_users, self.n_items], 0) # 在行上面分割
    #     return u_g_embeddings, i_g_embeddings
    ...
    def _create_ngcf_embed(self):
        ...
        for k in range(0, self.n_layers):
            ...
            bi_embeddings = tf.multiply(ego_embeddings, side_embeddings)  # 相同位置元素相乘
    ...
    def _convert_sp_mat_to_sp_tensor(self, X):
        coo = X.tocoo().astype(np.float32)  # 返回稀疏矩阵的coo_matrix形式
...
if __name__ == '__main__':
    ...
    *********************************************************
    ...
    print('norm_adj:', norm_adj, norm_adj.shape)
    print('mean_adj:', mean_adj, mean_adj.shape)
    print('plain_adj:', plain_adj, plain_adj.shape)
    print('pre_adj:', pre_adj, pre_adj.shape)
    if args.adj_type == 'plain':
        ...
        print('use the normalized adjacency matrix使用标准化邻接矩阵')
    elif args.adj_type == 'gcmc':
        config['norm_adj'] = mean_adj
        print('use the gcmc adjacency matrix')
    elif args.adj_type == 'pre':
        config['norm_adj'] = pre_adj
        print('use the pre adjcency matrix使用加权平均邻接矩阵')  # 输出的是这个
    else:
        config['norm_adj'] = mean_adj + sp.eye(mean_adj.shape[0])
        print('use the mean 平均 adjacency matrix')
    ...
    '''定义模型实例'''
    model = LBGCN(data_config=config, pretrain_data=pretrain_data)
